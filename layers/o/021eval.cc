...
cell* eval(cell* expr, cell* scope) {
  ...
  if (cdr(expr) != nil && !is_compiledfn(body(fn)))
    add_lexical_binding(new_sym("all_unevald_args"), cdr(expr), new_scope);
//?   cerr << expr << ": all_unevald_args is now " << lookup(new_sym("all_unevald_args"), new_scope) << '\n';
  if (is_quoted(sig(fn))) {
    TEMP(spliced_args, splice_quoted_args(cdr(expr), scope, fn, expr));
    trace("unevald_args") << spliced_args;
    bind_params(strip_quote(sig(fn)), spliced_args, new_scope);
  }
  else {
    ...
    if (cdr(expr) != nil && !is_compiledfn(body(fn)))
      add_lexical_binding(new_sym("all_args"), all_args(sig(fn), new_scope), new_scope);
//?     cerr << expr << ": all_args is now " << lookup(new_sym("all_args"), new_scope) << '\n';
  }
...
cell* all_args(cell* params, cell* scope) {
  if (params == nil) return nil;
  if (is_sym(params)) return lookup(params, scope);
  return new_cons(all_args(car(params), scope),
                  all_args(cdr(params), scope));
}
