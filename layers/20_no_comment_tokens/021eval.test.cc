...
void test_colonsym_evals_to_itself() {
  run(":abc");
  CHECK_TRACE_CONTENTS("eval", ":abc\n=> keyword sym: :abc\n");
}

void test_colon_evals() {
  newBinding(":", newNum(34));
  run(":");
  CHECK_TRACE_CONTENTS("eval", ":\n=> sym: 34\n");
}

void test_string_evals_to_itself() {
  run("\"ac bd\"");
  CHECK_TRACE_CONTENTS("eval", "\"ac bd\"\n=> literal: \"ac bd\"\n");
}
...
void test_eval_handles_quoted_lists() {
  run("'(a b)");
  CHECK_TRACE_CONTENTS("eval", "'(a b)\n=> quote: (a b)\n");
}

void test_eval_handles_rest_params() {
  run("((fn (a b ... c) c) 1 2 3 4 5)");
  CHECK_TRACE_CONTENTS("eval", 1, "((fn (a b ... c) c) 1 2 3 4 5)\n=> (3 4 5)\n");
}

void test_eval_handles_fn_calls() {
  run("((fn () 34))");
  CHECK_TRACE_CONTENTS("eval", 1, "((fn nil 34))\n=> 34\n");
}

void test_eval_expands_syms_in_fn_bodies() {
  newBinding("a", newNum(34));
  run("((fn () a))");
  CHECK_TRACE_CONTENTS("eval", 1, "((fn nil a))\n=> 34\n");
}
...
void test_eval_handles_multiple_args() {
  ...
  CLEAR_TRACE;
  run("(f 1 2)");
  CHECK_TRACE_CONTENTS("eval", 1, "(f 1 2)\n=> 2\n");
}
...
void test_eval_handles_vararg_param() {
  run("((fn args args) 1)");
  CHECK_TRACE_CONTENTS("eval", 1, "((fn args args) 1)\n=> (1)\n");
}

void test_eval_evals_args() {
  run("((fn (f) (f)) (fn () 34))");
  CHECK_TRACE_CONTENTS("eval", 1, "((fn (f) (f)) (fn nil 34))\n=> 34\n");
}

void test_eval_handles_destructured_params() {
  run("((fn ((a b)) b) '(1 2))");
  CHECK_TRACE_CONTENTS("eval", 1, "((fn ((a b)) b) '(1 2))\n=> 2\n");
}
