...
void test_if_sees_args_in_then_and_else() {
  ...
  CLEAR_TRACE;
  run("(f 35)");
  CHECK_TRACE_CONTENTS("eval", 1, "=> 35\n");
}

void test_not_works() {
  run("(not 35)");
  CHECK_TRACE_CONTENTS("eval", 1, "compiled fn\n=> nil\n");
}
...
void test_assign_lexical_var() {
  run("((fn (x) (<- x 34) x))");
  CHECK_TRACE_CONTENTS("eval", 1, "=> 34\n");
}
...
void test_assign_overrides_lexical_var() {
  ...
  CHECK_TRACE_CONTENTS("eval", 1, "=> 36\n");
}

void test_equal_handles_nil() {
  run("(= nil nil)");
  CHECK_EQ(traceCount("eval", 1, "compiled fn\n=> nil\n"), 0);
}
