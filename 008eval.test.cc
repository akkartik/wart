void test_bindArgs_handles_vararg() {
  Cell* params = wartRead(teststream(L"a")).front();
  Cell* args = wartRead(teststream(L"(1)")).front();
  newLexicalScope();
...
void test_nil_evals_to_itself() {
  list<Cell*> cells = wartRead(teststream(L"()"));
...
void test_num_evals_to_itself() {
  list<Cell*> cells = wartRead(teststream(L"34"));
  check_eq(cells.size(), 1);
  Cell* result = eval(cells.front());
...
void test_colonsym_evals_to_itself() {
  list<Cell*> cells = wartRead(teststream(L":abc"));
  check_eq(cells.size(), 1);
  Cell* result = eval(cells.front());
  check_eq(result, cells.front());
  rmref(result);
...
void test_string_evals_to_itself() {
  list<Cell*> cells = wartRead(teststream(L"\"ac bd\""));
  check_eq(cells.size(), 1);
  Cell* result = eval(cells.front());
  check_eq(result, cells.front());
  rmref(result);
...
void test_eval_handles_quoted_atoms() {
  list<Cell*> cells = wartRead(teststream(L"'a\n'34"));
...
void test_eval_handles_quoted_lists() {
  list<Cell*> cells = wartRead(teststream(L"'(a b)"));
  Cell* result = eval(cells.front());
...
void test_eval_handles_splice() {
  list<Cell*> cells = wartRead(teststream(L"`(a ,@b)"));
  newDynamicScope(L"b", wartRead(teststream(L"34 35")).front());
  Cell* result = eval(cells.front());
  check_eq(car(result), newSym(L"a"));
  check_eq(car(cdr(result)), newNum(34));
...
void test_eval_handles_splice_of_nil() {
  list<Cell*> cells = wartRead(teststream(L"`(a ,@b 3)"));
...
void test_eval_handles_simple_lambda() {
  list<Cell*> cells = wartRead(teststream(L"(lambda () 34)"));
  check_eq(cells.size(), 1);
...
void test_eval_handles_lambda_calls() {
  Cell* call = wartRead(teststream(L"((lambda () 34))")).front();
...
void test_eval_expands_syms_in_lambda_bodies() {
  Cell* lambda = wartRead(teststream(L"((lambda () a))")).front();
...
void test_eval_handles_assigned_lambda_calls() {
  Cell* lambda = wartRead(teststream(L"(lambda () 34)")).front();
  Cell* f = eval(lambda);
  newDynamicScope(L"f", f);
    Cell* call = wartRead(teststream(L"(f)")).front();
    Cell* result = eval(call);
    check_eq(result, newNum(34));
  endDynamicScope(L"f");
  rmref(result);
...
void test_eval_expands_lexically_scoped_syms_in_lambda_bodies() {
  Cell* call = wartRead(teststream(L"((lambda () a))")).front();
  newLexicalScope();
...
void test_eval_expands_args_in_caller_scope() {
  newDynamicScope(L"a", newNum(23));
  Cell* lambda = wartRead(teststream(L"(lambda (arg1) arg1)")).front();
  newLexicalScope();
  addLexicalBinding(newSym(L"arg1"), newNum(34));
    Cell* f = eval(lambda);
    newDynamicScope(L"f", f);
  endLexicalScope();
  Cell* call = wartRead(teststream(L"(f a)")).front();
  Cell* result = eval(call);
...
void test_eval_handles_multiple_args() {
  Cell* lambda = wartRead(teststream(L"(lambda (a b) b)")).front();
  Cell* f = eval(lambda);
  newDynamicScope(L"f", f);
  Cell* call = wartRead(teststream(L"(f 1 2)")).front();
  Cell* result = eval(call);
...
void test_eval_handles_multiple_body_exprs() {
  Cell* lambda = wartRead(teststream(L"(lambda () 1 2)")).front();
  Cell* f = eval(lambda);
  newDynamicScope(L"f", f);
  Cell* call = wartRead(teststream(L"(f)")).front();
  Cell* result = eval(call);
...
void test_eval_handles_vararg_param() {
  Cell* call = wartRead(teststream(L"((lambda args args) 1)")).front();
  Cell* result = eval(call);
  check(isCons(result));
  check_eq(car(result), newNum(1));
  rmref(result);
...
void test_eval_evals_args() {
  Cell* call = wartRead(teststream(L"((lambda (f) (f)) (lambda () 34))")).front();
  Cell* result = eval(call);
  check(isNum(result));
...
void test_eval_doesnt_leak_body_evals() {
  Cell* call = wartRead(teststream(L"((lambda (f) (f) (f)) (lambda () 34))")).front();
  Cell* result = eval(call);
  check(isNum(result));
  check_eq(toNum(result), 34);
  rmref(result);
...
void test_eval_handles_destructured_params() {
  Cell* call = wartRead(teststream(L"((lambda ((a b)) b) '(1 2))")).front();
  Cell* result = eval(call);
  check(isNum(result));
...
void test_eval_handles_rest_params() {
  Cell* call = wartRead(teststream(L"((lambda (a b . c) c) 1 2 3 4 5)")).front();
  Cell* result = eval(call);
  check(isCons(result));
...
void test_eval_doesnt_modify_lambda() {
  Cell* lambda = wartRead(teststream(L"(lambda(x) (eval x))")).front();
  Cell* f = eval(lambda);
  newDynamicScope(L"f", f);
  Cell* oldf = copyList(f);
  Cell* call = wartRead(teststream(L"(f 34)")).front();
  Cell* result = eval(call);
...
void test_eval_doesnt_modify_lambda2() {
  ...
  newDynamicScope(L"f", f);
  Cell* oldf = copyList(f);
  Cell* call = wartRead(teststream(L"(f '(cons 3 4))")).front();
  Cell* result = eval(call);
  check(equalList(f, oldf));
  rmref(result);
  rmref(call);
...
void test_eval_doesnt_modify_lambda3() {
  ...
  Cell* lambda = wartRead(teststream(L"(lambda(x y) `(assign ,x ,y))")).front();
  Cell* f = eval(lambda);
  newDynamicScope(L"f", f);
  Cell* oldf = copyList(f);
  Cell* call = wartRead(teststream(L"(f a b)")).front();
  Cell* result = eval(call);
  check(equalList(f, oldf));
  rmref(result);
  rmref(call);
...
void test_eval_doesnt_modify_lambda4() {
  newDynamicScope(L"a", newNum(3));
  newDynamicScope(L"b", newNum(4));
  Cell* lambda = wartRead(teststream(L"(lambda y `(assign ,@y))")).front();
  Cell* f = eval(lambda);
  newDynamicScope(L"f", f);
  Cell* oldf = copyList(f);
  Cell* call = wartRead(teststream(L"(f a b)")).front();
...
void test_eval_handles_eval() {
  ...
  Cell* call = wartRead(teststream(L"(eval x)")).front();
  Cell* result = eval(call);
  check_eq(result, newNum(34));
  rmref(result);
  rmref(call);
