void test_countIndent() {
  ...
  check_eq(countIndent(stream(L"\t")), 1);
  check_eq(countIndent(stream(L" ")), 1+LAST_CHAR_IS_SPACE);
  check_eq(countIndent(stream(L"   ")), 3+LAST_CHAR_IS_SPACE);
  check_eq(countIndent(stream(L" \t ")), 3+LAST_CHAR_IS_SPACE); // tab == 1 space
  check_eq(countIndent(stream(L" \n ")), 1+LAST_CHAR_IS_SPACE); // skip empty lines
