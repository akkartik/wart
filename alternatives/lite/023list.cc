...
COMPILE_FN(list_splice, compiledfn_list_splice, "'($list $start $end $val)",
  ...
  TEMP(listc, eval(binding));
  TEMP(startc, eval(lookup("$start")));
  long start = to_int(startc);
  cell* pre_ptr = nth_cdr(listc, start-1);
  cell* start_ptr = nth_cdr(listc, start);
  TEMP(endc, eval(lookup("$end")));
  TEMP(end_ptr, mkref(nth_cdr(listc, to_int(endc))));
