...
cell* eval(cell* expr, cell* scope) {
  ...
  TEMP(params, mkref(sig(fn)));
  if (is_alias(strip_quote(params))) {
    cell* aliases = cdr(strip_quote(params));
    if (len(aliases) != 2)
      RAISE << "currently only two-param aliases are supported: " << params << '\n';
    cell* first_alias = car(aliases);
    update(params, mkref(car(cdr(aliases))));
    if (is_quoted(sig(fn)) && !is_quoted(params))
      update(params, mkref(new_cons(sym_quote, params)));
    if ((!is_quoted(sig(fn)) && !is_quoted(first_alias))
        || !is_sym(strip_quote(first_alias)))
      RAISE << "first param alias must be a quoted sym: " << params << '\n';
    add_lexical_binding(strip_quote(first_alias), cdr(expr), new_scope);
  }
  if (is_quoted(params)) {
    TEMP(spliced_args, splice_quoted_args(cdr(expr), scope, fn, expr));
    trace("unevald_args") << spliced_args;
    bind_params(strip_quote(params), spliced_args, new_scope);
  }
  else {
    TEMP(spliced_args, splice_args(cdr(expr), scope));
    trace("unevald_args") << spliced_args;
    eval_bind_all(params, spliced_args, scope, new_scope);
  }
...
void eval_bind_all(cell* params, cell* args, cell* scope, cell* new_scope) {
  ...
  if (params == nil)
    ...
    eval_bind_rest(params, args, scope, new_scope);
...
void eval_bind_rest(cell* param, cell* args, cell* scope, cell* new_scope) {
  ...
  if (is_cons(param))
    ...
    TEMP(cached_val, eval_all(args, scope));
    bind_params(param, cached_val, new_scope);
  }
}
...
cell* splice_args(cell* args, cell* scope) {
...
cell* splice_quoted_args(cell* args, cell* scope, cell* fn, cell* expr) {
  cell *p_result = new_cell(), *tip = p_result;
  for (cell* curr = args; curr != nil; curr=cdr(curr)) {
    if (!is_spliced(car(curr))) {
      add_cons(tip, car(curr));
    ...
    if (is_macro(fn))
      RAISE << "calling macros with splice can have subtle effects (http://arclanguage.org/item?id=15659): " << expr << '\n';
    TEMP(x, unsplice(car(curr), scope));
    for (cell* curr2 = x; curr2 != nil; curr2=cdr(curr2), tip=cdr(tip))
      add_cons(tip, car(curr2));
  }
  trace("splice") << cdr(p_result);
  return drop_ptr(p_result);
}
